...
export const instagramSchedulingService = {
  ...
  /**
   * Get OAuth URL to initiate Instagram connection.
   * Returns the Meta OAuth URL that the user should be redirected to.
   */
  async getOAuthUrl(): Promise<{ url: string }> {
    const response = await api.get<{ url: string }>(
      `${BASE_URL}/instagram/connect/`
    );
   ...
   * Exchanges the code for access token and creates/updates the Instagram account.
   */
  async processOAuthCallback(code: string): Promise<InstagramAccount> {
    const response = await api.post<InstagramAccount>(
